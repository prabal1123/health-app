...
export async function fetchGoogleFitSteps(userId) {
  try {
    ...
    // ✅ Use the full function URL (Option B)
    const SYNC_URL = import.meta.env.VITE_SYNC_GOOGLE_FIT_STEPS_URL;
    if (!SYNC_URL) throw new Error('Backend function URL is not configured. (VITE_SYNC_GOOGLE_FIT_STEPS_URL missing)');
    ...
    const response = await fetch(SYNC_URL, {
      ...
      headers: {
        'Content-Type': 'application/json',
        // Send the user's access token so the function can identify the user
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify({ user_id: userId, userTimeZone }),
    });
